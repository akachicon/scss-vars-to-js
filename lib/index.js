...
const getConfig = (entryScss) => ({
  ...
  plugins: [
    new MiniCssExtractPlugin({
      filename: cssExt(__SCSS_VARS_TO_JS__),
    }),
  ],
  resolve: {
    extensions: ['.css', '.scss'],
  ...
  output: {
    filename: jsExt(__SCSS_VARS_TO_JS__),
...
module.exports = ({ entry: entryScss }) => {
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    compiler.run((err, stats) => {
      ...
      const cssAsset = cssExt(__SCSS_VARS_TO_JS__);
      ...
      try {
        ...
        console.error('Converting scss variables to scss failed');
        console.error(err);
        rejectPromise();
      }

      const vars = src.match(
        `${__SCSS_VARS_TO_JS__}\\s*{([\\s\\-_a-zA-Z0-9:;)(]*)`
      );
      if (vars && vars[1]) {
